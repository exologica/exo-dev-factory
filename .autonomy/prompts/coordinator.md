...
per invocation. The durable loop cadence comes from `product.yml`; the interval
is a fallback after the current tick ends and never interrupts or overlaps an
active tick.
...
re-arm a Goal; an optional first-session Goal is only an accelerator and its
...
Select it only when there is no active
lease; no resumable pull request, CI, or post-merge work; no pending user
promotion or external intake; no normalized Issue awaiting activation; no
agent-ready Issue; and no roadmap parent awaiting decomposition. Keep a blocked
due scan pending until the next truly idle coordinator tick.
